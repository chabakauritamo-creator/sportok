...
export function AuthCard({ mode, locale, t }: { mode: Mode; locale: string; t: Dictionary }) {
  ...
  const handle = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setPending(false);
      setMsg(locale === 'ka' ? 'მონაცემთა ბაზა ჯერ არ არის დაკავშირებული.' : 'Backend not wired yet — this is the MVP UI.');
    }, 600);
  };
  ...
  return (
    <div className="card mx-auto w-full max-w-md p-6 md:p-8">
      ...
      <form onSubmit={handle} className="space-y-4">
        ...
        {msg && <div className="rounded-md border border-[var(--color-border-strong)] bg-[var(--color-surface-2)] p-3 text-[12px] text-[var(--color-text-muted)]">{msg}</div>}
      </form>
